...
def find_parameters():  # параметры поиска
    print('По какому параметру Вы бы хотели выполнить поиск?')
    ...
    while True:
        if search_field == '1':
            ...
            print('Некорректный выбор поля поиска! Попробуйте еще раз.')
            print()
            continue
...
def change_phonebook(file_name): # изменение данных
    ...
    with open(file_name, 'w', encoding='utf-8') as file:
        for contact in contact_list:
            ...
            file.write(line)
    print()
    print('Отредактированный справочник')
...
def delete_contacts(file_name):
    ...
    print_contacts(contact_list)
    number_of_del = int(input('Введите номер контакта который хотите удалить (0 - отмена): '))
...
def read_file_to_list(file_name):  # считываем Phonebook1.txt в массив
